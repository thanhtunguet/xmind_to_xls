...
class xmind_to_excel(object):
    ...
    def __init__(self, xmind_path, xmind_file, name):
        ...
        self.xls_path = xmind_path + './' + name + '.xls'
...
class style_excel(object):
    ...
    def do_merge_excel(self, num_start, num_end, column, text):
        ...
        self.style_worksheet.write_merge(
            num_start+1, num_end+1, column, column, text, self.style)
                        ...
                        try:
                            for c in range(0, merge_row[2]):
                                ...
                                if 'text' in str(self.data.col(c)[o+1]) and self.end_col(merge_row[0], self.data.col(merge_row[2]+1)):
                                    self.do_merge_excel(
                                        merge_row[0], merge_row[0]+(r-1), merge_row[2], merge_row[3])
                                    k = 1
                                elif merge_row[0] + r == merge_row[1] and c >= merge_row[2]-1 and o >= merge_row[1]-1:
                                    self.do_merge_excel(
                                        merge_row[0], merge_row[1], merge_row[2], merge_row[3])
                                    k = 1
...
def main():
    argv = sys.argv[1:]
    print(argv)
    input_file, name = argv

    a = xmind_to_excel('', input_file, name)
    a.to_excel(a.data_dict[0]['topic'])
    a.save_xls()
    filename = name + '.xls'
    b = style_excel('./', filename, a.data_dict[0]['topic']['title'])
    b.merge_excel(b.calculate())
    b.save_style_excel(filename)
